...
aurora.get("/hello", function(request)
    local data = {
        title = "Hello World",
    ...
    return aurora.sendHtml("index.html", data) -- Path is relative to the working directory
end)

aurora.createServer({}) -- http://localhost:8080/hello
```
...
aurora.get("/hello", function(request)
    ...
    return aurora.sendHtml("index.html", data)
end)
